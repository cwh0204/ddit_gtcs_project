...
// src/style/layout/common/WarehouseSidebar.jsx

/**
 * 창고관리자 전용 사이드바
 */
function WarehouseSidebar({ menuStructure, title, footer }) {
  ...
  return (
    <aside className="relative flex flex-col w-64 h-full border-r border-[#123150] bg-[#0a2742] text-white overflow-y-auto shrink-0">
      <nav className="flex flex-col gap-1 pb-20">
        {/* 상단 타이틀 */}
        <p className="px-4 text-xs font-semibold text-gray-400 mb-2 mt-4">{title}</p>
            ...
            <div key={item.id} className="flex flex-col">
              ...
              <button
                ...
                className={cn(
                  ...
                  isMainActive && !hasActiveSubItem ? "text-white font-bold" : "text-gray-300 hover:bg-[#123150] hover:text-white",
                )}
              >
                <span>{item.name}</span>
              ...
              {isOpen && (
                <div className="flex flex-col gap-1 pl-4 mt-1 mb-2 border-l border-gray-600 ml-6">
                  {item.subItems.map((sub) => {
                    ...
                    const isSubActive = location.pathname === fullPath || location.pathname.startsWith(`${fullPath}/`);
        ...
        <div className="mx-2 mt-4 mb-2">
          <div className="flex items-center justify-between px-4 py-3 rounded-lg bg-[#123150]/50 hover:bg-[#123150] transition-colors">
            <div className="flex items-center gap-2 min-w-0">
              <span className="text-sm font-medium text-white truncate">{user?.name || "창고관리자"}</span>
            </div>
            <button onClick={handleLogout} className="flex-shrink-0 p-1.5 text-gray-400 hover:text-white rounded transition-colors" title="로그아웃">
      ...
      <div className="absolute bottom-4 left-0 w-full px-6 text-xs text-gray-400 bg-[#0a2742] pt-2 border-t border-[#123150]">
        ...
        <p className="mt-1 opacity-70">{footer}</p>
      </div>
    </aside>
  );
}
