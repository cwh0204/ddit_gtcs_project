...
/* 날짜 입력창 크기를 일시(datetime-local)에 맞게 살짝 넓힘 */
.search-bar input[type="datetime-local"].form-input {
	width: 175px;
	font-family: inherit;
}
...
/* ===== 페이지네이션 맨 끝 화살표(>>) 아이콘 깨짐/색상 강제 수정 ===== */
[id$="_pagination"] button:last-child,
[id$="_pagination"] button:last-child i {
    color: #666 !important; /* 기본 색상을 진한 회색으로 강제 지정 */
}

[id$="_pagination"] button:last-child:disabled,
[id$="_pagination"] button:last-child:disabled i {
    color: #ccc !important; /* 버튼이 비활성화(끝 페이지 도달) 시 연한 회색 */
}

/* 폰트어썸 아이콘이 깨졌을 경우를 대비해 '>>' 아이콘(\f101) 강제 생성 */
[id$="_pagination"] button:last-child i::before {
...
<div class="ag-grid-container">
	...
	<div class="page-header">
		<div>
			<h2 class="page-title">화물 진행 현황</h2>
			<div class="breadcrumb-text">
				<i class="fas fa-home"></i> Home &gt; 화물관리 &gt; 화물 진행 현황
			</div>
		</div>
		<div class="toggle-group">
			<button type="button" class="toggle-btn active" id="btn-all"
				onclick="changeCargoType('all')">전체 화물</button>
			<button type="button" class="toggle-btn" id="btn-undecl"
				onclick="changeCargoType('undecl')">미신고 화물</button>
			<button type="button" class="toggle-btn" id="btn-import"
				onclick="changeCargoType('import')">수입 화물</button>
			<button type="button" class="toggle-btn" id="btn-export"
				onclick="changeCargoType('export')">수출 화물</button>
		</div>
	</div>

	<div class="search-bar">
        <span class="search-label">최초등록일시</span> 
	...
	<div class="stats-bar" id="statsBar">
		<i class="fas fa-layer-group"></i> 총 <span class="stats-value" id="totalCountDisplay">0</span>건
	</div>

	<jsp:include page="/WEB-INF/components/AgGridList.jsp">
		<jsp:param name="gridId" value="cargoTrackingGrid" />
...
// [1] 전역 변수
// =========================================================
const GRID_ID          = 'cargoTrackingGrid';
...
const loadPageData = (isBackground) => {
    ...
    console.log("1. 현재 찾은 그리드 API:", api);
    
    if (!api) { 
        console.log("2. 그리드 준비 안됨! 0.1초 뒤 재시도...");
        setTimeout(() => loadPageData(isBackground), 100); 
        return; 
    }
    
    console.log("3. 그리드 준비 완료! Axios 요청 시작합니다.");
