...
function renderNewFileList() {
    ...
    container.innerHTML = fileBuffer.map((file, index) =>
        `<div style="display: flex; align-items: center;">     
            <span class="file-link" style="margin-right: 10px;">${file.name}</span>
            <button type="button" onclick="removeNewFile(${index})" 
                style="color: red; border: none; background: none; cursor: pointer; font-size: 14px; padding: 0 5px;" title="취소">
...
function displayBoardDetail(board) {
    ...
    if (isEditMode) {
        ...
        onmouseover="this.style.backgroundColor='#0f4c81'; this.style.color='#fff';"
        onmouseout="this.style.backgroundColor='#fff'; this.style.color='#0f4c81';"
        style="padding: 6px 12px; cursor: pointer; border: 1px solid #0f4c81; background: #fff; color: #0f4c81; border-radius: 4px; font-weight: 600; transition: 0.2s;">
    + 파일 추가
</button>
                </div>
        ...
        if (board.fileList && board.fileList.length > 0) {
   		    fileHtml += `<div style="display: flex; flex-direction: column; gap: 5px;">`;
            fileHtml += board.fileList.map(file =>
                `<div id="file_${file.fileId}" style="display: flex; align-items: center;">
                    ...
                    <button type="button" onclick="removeExistingFile('${file.fileId}')"
                        style="color: red; border: none; background: none; cursor: pointer; font-size: 14px; padding: 0 5px;" title="삭제">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
                </div>`
            ).join('');
