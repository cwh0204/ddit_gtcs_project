/**
 * 행정예고 등록/수정 - axios + async/await + JWT 방식
 */
...
document.querySelector('#writeForm').addEventListener('submit', async function(e) {
    ...
    if(!confirm('행정예고를 등록하시겠습니까?')) return;
    ...
    try {
        ...
        // 3. 파일 정보를 맵(Map) 구조처럼 개별 키로 추가 (핵심 수정 부분)
        // 서버의 Map<String, MultipartFile> fileMap에서 
        // 'file0', 'file1' 등이 Map의 Key가 됩니다.
        if (typeof fileBuffer !== 'undefined' && fileBuffer.length > 0) {
            fileBuffer.forEach((file, index) => {
                // 'file[index]' 대신 'file0', 'file1' 처럼 고유 키 할당
                formData.append(`file${index}`, file); 
            });
        }
        
        // 4. Axios 전송 (직접 설정을 통한 멀티파트 보장)
        const response = await axios({
            ...
            headers: {
                ...
                // 수동으로 명시하되, Axios가 boundary를 잃지 않도록 설정
